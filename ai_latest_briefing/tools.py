...
def build_and_save_html(topic: str = "AI", tool_context=None) -> dict:
    ...
    # Deduplicate globally across all cards — each URL appears at most once in the whole briefing
    global_seen: set = set()
    ...
    print(f"Publisher -- building HTML with {sum(len(i.get('urls', [])) for i in news_items)} unique source links")

    html = _build_html(tldr, news_items, community_pulse, topic,
                       tldr_he, news_items_he, community_pulse_he,
                       community_urls)
    ...
    path = os.path.join(out_dir, f"briefing_{ts}.html")
...
def _vendor_style(vendor: str):
    ...
    for k, v in _VENDOR_COLORS.items():
        if k in key:
            return v
    return ("#6b7280", "#f9fafb")  # Other/gray
...
def _build_html(tldr, news_items, community_pulse, topic,
    ...
    # News cards (EN + HE side by side, toggled by JS)
    cards = ""
    for idx, item in enumerate(news_items):
        ...
        date_html = f'<span class="pub-date">📅 {published_date}</span>' if published_date else ""
        sources_html = "".join(
            f'<a href="{u}" target="_blank" class="source-link">{u[:65]}{"..." if len(u) > 65 else ""}</a>'
            for u in urls if u
        )
...
<title>{topic} Latest Briefing - {date_display}</title>
<style>
...
.header{{background:linear-gradient(135deg,#0f0c29,#302b63,#24243e);color:#fff;padding:32px 28px;text-align:center}}
.header h1{{font-size:24px;font-weight:700;margin-bottom:4px}}
.header .date{{font-size:14px;opacity:.75;margin-top:6px}}
.header .subtitle{{font-size:13px;opacity:.5;margin-top:2px}}
...
.tbtn.active{{background:#fff;color:#302b63}}
...
.tldr-card{{background:#fff;border-radius:12px;padding:20px 24px;margin-bottom:24px;border-left:4px solid #7c3aed;box-shadow:0 1px 3px rgba(0,0,0,.08)}}
.tldr-card h2{{font-size:16px;font-weight:700;color:#7c3aed;margin-bottom:12px}}
...
.card-header h3{{font-size:15px;font-weight:600;color:#1a1a2e;flex:1}}
.summary{{font-size:14px;line-height:1.7;color:#4b5563;margin-bottom:8px}}
.pub-date{{display:block;font-size:12px;color:#9ca3af;margin-bottom:8px}}
.sources{{display:flex;flex-direction:column;gap:3px;margin-top:8px;padding-top:8px;border-top:1px solid #f3f4f6}}
.source-link{{font-size:12px;color:#2563eb;text-decoration:none;word-break:break-all}}
.source-link:hover{{text-decoration:underline}}
.community-card{{background:#fff;border-radius:12px;padding:20px 24px;margin-bottom:24px;border-left:4px solid #0f172a;box-shadow:0 1px 3px rgba(0,0,0,.08)}}
.community-card h2{{font-size:16px;font-weight:700;color:#0f172a;margin-bottom:10px}}
.community-card p{{font-size:14px;line-height:1.7;color:#374151}}
.footer{{text-align:center;padding:20px;font-size:12px;color:#9ca3af}}
</style></head><body>
<div class="header">
<h1>Your {topic} Latest Briefing</h1>
<div class="date">{date_display}</div>
<div class="subtitle">Auto-generated by AI Latest Briefing Agent</div>
<div class="toggle">
...
<p id="community-en">{community_pulse}</p>
<p id="community-he" style="display:none;direction:rtl;text-align:right">{community_pulse_he}</p>
{community_sources_block}
</div>
</div>
<div class="footer">Generated on {now.strftime('%B %d, %Y at %H:%M')} | AI Latest Briefing Agent</div>
<script>
