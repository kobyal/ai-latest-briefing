...
Run exactly 5 searches — one per vendor. Use broad queries WITHOUT a specific date so the \
search engine surfaces the most recent results:
...
- Published: [exact date from the article, e.g. "March 22, 2026"]
...
1. tldr: EXACTLY 3 bullets — no more, no fewer. Each names the vendor + what happened + why it matters (15-25 words each).

2. news_items: 4-8 items. For each:
   - vendor: one of "Anthropic", "AWS", "OpenAI", "Google", "Azure", or "Other"
...
3. community_pulse: 2-3 sentences on what developers are actually saying. Be specific — reference the reactions found.
...
- tldr_he: list of 3 Hebrew bullet strings (translated from tldr)
